...
function createKeyDoubleMap(registry, _ref3, rawKey, args, _ref4) {
  let {
    meta: {
      name,
      type
  ...
  const param1Encoded = (0, _util.u8aConcat)(rawKey, (0, _codec.createTypeUnsafe)(registry, type1, [key1]).toU8a(true));
  ...
  const param2Hashed = key2Hasher((0, _codec.createTypeUnsafe)(registry, type2, [key2]).toU8a(true)); // as per createKey, always add the length prefix (underlying it is Bytes)
...
function createKey(registry, _ref5, rawKey, arg, hasher) {
  let {
    meta: {
      name,
      type
  ...
  if (type.isMap) {
    ...
    const param = (0, _codec.createTypeUnsafe)(registry, mapType, [arg]).toU8a();
...
function extendLinkedMap(registry, _ref7, storageFn, stringKey, hasher) {
  let {
    meta: {
      documentation,
      name,
  ...
  const headHash = new _codec.U8a(registry, hasher("head of ".concat(stringKey)));

  const headFn = () => headHash; // metadata with a fallback value using the type of the key, the normal
  // meta fallback only applies to actual entry values, create one for head


  headFn.meta = (0, _codec.createType)(registry, 'StorageEntryMetadataLatest', {
    name,
    modifier: (0, _codec.createType)(registry, 'StorageEntryModifierLatest', 1),
    // required
    type: (0, _codec.createType)(registry, 'StorageEntryTypeLatest', (0, _codec.createType)(registry, 'PlainTypeLatest', type.asMap.key), 0),
    fallback: (0, _codec.createType)(registry, 'Bytes', (0, _codec.createTypeUnsafe)(registry, type.asMap.key.toString()).toHex()),
    documentation
  ...
  storageFn.headKey = (0, _codec.createType)(registry, 'StorageKey', headFn, {
...
function createFunction(registry, item) {
  ...
  const _storageFn = arg => type.isDoubleMap ? createKeyDoubleMap(registry, item, rawKey, arg, [hasher, key2Hasher]) : createKey(registry, item, rawKey, arg, options.skipHashing ? NULL_HASHER : hasher);
