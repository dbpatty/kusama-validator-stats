...
function queryElections(api) {
  return api.query.elections.nextVoterSet().pipe((0, _operators.switchMap)(nextVoterSet => api.query.elections.voters.multi([...Array(+nextVoterSet + 1).keys()].map((_, i) => [i]))), (0, _operators.map)(voters => {
      ...
      vec.forEach((e, index) => {
        ...
        if (accountId) {
          result[accountId.toString()] = {
            ...
            setIndex: (0, _types.createType)(api.registry, 'SetIndex', setIndex)
          };
        }
      });
      return result;
