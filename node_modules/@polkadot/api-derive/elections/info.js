...
function deriveElections(api, _ref) {
  ...
  return {
    ...
    members: members.map((_ref2) => {
      let [accountId] = _ref2;
      return [accountId, (0, _types.createType)(api.registry, 'Balance')];
    }),
    runnersUp: [],
    termDuration,
    voteCount,
...
function queryElections(api) {
  ...
  return (0, _rxjs.combineLatest)([api.query.elections.candidates(), api.queryMulti([api.query.elections.candidateCount, api.query.elections.desiredSeats, api.query.elections.members, api.query.elections.nextVoterSet, api.query.elections.termDuration, api.query.elections.voteCount, api.query.elections.voterCount])]).pipe((0, _operators.map)(result => deriveElections(api, result)));
}

function derivePhragmen(api, candidates, members, runnersUp, candidacyBond, desiredSeats, termDuration, votingBond) {
  return {
    candidates,
...
function queryPhragmen(api) {
  ...
  return (0, _rxjs.combineLatest)([api.query.electionsPhragmen.candidates(), api.query.electionsPhragmen.members(), api.query.electionsPhragmen.runnersUp()]).pipe((0, _operators.map)((_ref3) => {
    ...
    return derivePhragmen(api, candidates, members, runnersUp, api.consts.electionsPhragmen.candidacyBond, api.consts.electionsPhragmen.desiredMembers, api.consts.electionsPhragmen.termDuration, api.consts.electionsPhragmen.votingBond);
  }));
}
/**
 * @name info
