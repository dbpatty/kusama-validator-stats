...
function unwrapStorageType(type) {
  if (type.isDoubleMap) {
    return "DoubleMap<".concat(type.asDoubleMap.value.toString(), ">");
  }

  if (type.isMap) {
    if (type.asMap.linked.isTrue) {
      return "(".concat(type.asMap.value.toString(), ", Linkage<").concat(type.asMap.key.toString(), ">)");
    }
