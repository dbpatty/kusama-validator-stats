...
function entrySignature(registry, storageEntry, imports) {
  if (storageEntry.type.isPlain) {
    (0, _util2.setImports)(imports, [storageEntry.type.asPlain.toString()]);
    return ['', (0, _util2.formatType)(storageEntry.type.asPlain.toString(), imports)];
  } else if (storageEntry.type.isMap) {
    // Find similar types of the `key` type
    const similarTypes = (0, _util2.getSimilarTypes)(registry, storageEntry.type.asMap.key.toString(), imports);
    ...
    const similarTypes1 = (0, _util2.getSimilarTypes)(registry, storageEntry.type.asDoubleMap.key1.toString(), imports);
    const similarTypes2 = (0, _util2.getSimilarTypes)(registry, storageEntry.type.asDoubleMap.key2.toString(), imports);
...
function generateEntry(registry, storageEntry, imports) {
...
function generateModule(registry, modul, imports) {
  ...
  return [(0, _util2.indent)(4)("".concat((0, _util.stringLowerFirst)(modul.name.toString()), ": {"))].concat((0, _util2.indent)(6)('[index: string]: QueryableStorageEntry<ApiType>;')).concat(modul.storage.unwrap().items.reduce((acc, storageEntry) => {
    return acc.concat(generateEntry(registry, storageEntry, imports).map((0, _util2.indent)(6)));
  }, []).join('\n')).concat([(0, _util2.indent)(4)('};')]);
} // Generate `packages/types-jsonrpc/src/jsonrpc.types.ts` for a particular
// metadata


function generateForMeta(registry, meta) {
...
function generateQuery() {
  ...
  return generateForMeta(registry, new _.Metadata(registry, _static.default));
}